...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, proof_offset, 
                 node_label, edge_label, label_id, relevant_nodes=None):
...
def convert_examples_to_features_RR(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # Get relevant nodes for this example
        context_nodes = example.context.split(". ")
        relevant_nodes = get_relevant_nodes(example.question, context_nodes)
        
        # Process only relevant nodes instead of all sentences
        context_tokens = []
        proof_offset = []
        for node in relevant_nodes:
            node_tokens = tokenizer.tokenize(node)
            context_tokens.extend(node_tokens)
...
def get_relevant_nodes(question, context_nodes):
    """
    Extract relevant nodes based on question keywords
    Args:
        question: str, the question text
        context_nodes: list of str, all available context nodes (rules and facts)
    Returns:
        relevant_nodes: list of str, filtered relevant nodes
    """
    ...
    # Get relevant words (nouns, verbs, adjectives)
    relevant_words = set()
    for word, tag in pos_tags:
        if tag.startswith(('NN', 'VB', 'JJ')):
            relevant_words.add(stemmer.stem(word.lower()))
    
    # Track processed nodes to avoid duplicates
    processed_nodes = set()
    relevant_nodes = []
    words_to_process = list(relevant_words)
    
    while words_to_process:
        current_word = words_to_process.pop(0)
        if current_word in processed_nodes:
            continue
            
        processed_nodes.add(current_word)
        
        # Find nodes containing the current word
        for node in context_nodes:
            node_words = set(stemmer.stem(w.lower()) for w in word_tokenize(node))
            
            if current_word in node_words:
                if node not in relevant_nodes:
                    relevant_nodes.append(node)
                    # Add new words from this node to process
                    for word, tag in pos_tag(word_tokenize(node)):
                        if tag.startswith(('NN', 'VB', 'JJ')):
                            stem = stemmer.stem(word.lower())
                            if stem not in processed_nodes:
                                words_to_process.append(stem)
