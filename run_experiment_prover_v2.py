...
from model_v2 import RobertaForRRWithNodeEdgeLoss
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter(log_dir=f"runs_v2/{args.output_dir.split('/')[-1]}")
...
def evaluate(args, model, tokenizer, processor, prefix="", eval_split="train"):
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_split="train"):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features_RR(examples, label_list, args.max_seq_length,
                                                 args.max_node_length, args.max_edge_length,
    ...
    # Add number of relevant nodes to the dataset
    all_num_relevant_nodes = torch.tensor([len(f.relevant_nodes) for f in features], dtype=torch.long)
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, 
                           all_proof_offset, all_node_label, all_edge_label,
                           all_label_ids, all_num_relevant_nodes)
...
def main():
    ...
    parser.add_argument("--output_dir", default="./output_v2/d5", type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")

    parser.add_argument("--data_cache_dir", default="./output_v2/cache/", type=str,
                        help="The directory where cached files will be written.")
