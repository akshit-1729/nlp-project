...
class RobertaForRRWithNodeEdgeLoss(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, proof_offset=None, node_label=None,
                edge_label=None, labels=None, position_ids=None, head_mask=None, num_relevant_nodes=None):
        outputs = self.roberta(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask)
        ...
        for batch_index in range(batch_size):
            ...
            # Only create edges between relevant nodes
            if num_relevant_nodes is not None:
                n_nodes = num_relevant_nodes[batch_index]
            else:
                n_nodes = len(sample_node_embedding)

            repeat1 = sample_node_embedding[:n_nodes].unsqueeze(0).repeat(n_nodes, 1, 1)
            repeat2 = sample_node_embedding[:n_nodes].unsqueeze(1).repeat(1, n_nodes, 1)
